...
def train_neg(negfile,num,hpo_dic,outpath):
    ...
    fout=open(outpath+'weak_train_neg.conll','w',encoding='utf-8')
...
def combine_pos_neg(outpath):
    fin_pos=open(outpath+'weak_train_pos.conll','r',encoding='utf-8')
    fin_neg=open(outpath+'weak_train_neg.conll','r',encoding='utf-8') 
    fout=open(outpath+'weak_train.conll','w',encoding='utf-8')
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description='build weak training corpus, python build_dict.py -d dictpath -f fileneg  -n number of neg -o outpath')
    parser.add_argument('--dict', '-d', help="the input path of the ontology dictionary",default='../dict/')
    parser.add_argument('--fileneg', '-f', help="the text file used to generate the negatives",default='../mutation_disease_1990_test.txt')
    parser.add_argument('--negnum', '-n', help="the number of negatives ",type=int, default=100)
